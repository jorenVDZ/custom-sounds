...
public class SoundEngine
{
	...
	private boolean loadClip(Sound sound)
	{
		try (InputStream stream = new BufferedInputStream(SoundFileManager.getSoundStream(sound)))
		{
			...
			{
				clip.open(audioInputStream); // liable to error with pulseaudio, works on windows, one user informs me
				// mac works
			}
			return true;
		}
		...
		{
			log.warn("Failed to load Custom Sounds sound " + sound, e);
		}
		return false;
	}
	...
	public void playClip(Sound sound)
	{
		...
		if (clip == null || currentMTime != lastClipMTime || !clip.isOpen())
		{
			...
			catch (LineUnavailableException e)
			{
				...
				log.warn("Failed to get clip for Custom Sounds sound " + sound, e);
				return;
			}
